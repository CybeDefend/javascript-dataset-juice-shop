...
function loadEnhancedHint(hint: ChallengeHint, sessionId: string): HTMLElement {
    ...
    const enhancedText = sessionManager.processSessionHint(sessionId, hint.text)
    textBox.innerHTML = snarkdown(enhancedText)
...
export async function startEnhancedHackingInstructorFor(challengeName: string, sessionId: string, customPrefs?: string): Promise<void> {
    sessionManager.initializeSession(sessionId, navigator.userAgent, customPrefs)
    ...
    for (const hint of challengeInstruction.hints) {
        const element = loadEnhancedHint(hint, sessionId)
        if (!element) {
